...
export default function Navbar() {
  ...
  const [isBlurSupported, _] = useState(browserName !== 'Safari' && browserName !== 'Mobile Safari')

  return (
    <div className="flex justify-between items-center">
      <span className="text-3xl whitespace-nowrap">Franky's Chronicles</span>
      <button onClick={() => setIsMenuOpen(true)}>
        <Image src="/menu.svg" alt="menu button" width={37} height={37} className="self-end min-w-fit" />
      </button>
      <AnimatePresence>
        {isMenuOpen && (
          <div className="absolute top-0 left-0 w-full h-full z-40">
            <motion.div className={`absolute inset-0 -z-10 ${isBlurSupported ? '' : 'bg-dark bg-opacity-95'}`} key={1}
              initial={{ opacity: 0, backdropFilter: 'blur(0px)' }}
              animate={{ opacity: 1, backdropFilter: 'blur(10px)', transition: { duration: 0.5 }}}
              exit={{ opacity: 0, backdropFilter: 'blur(0px)', transition: { duration: .5 }}}
            />
            <div className="flex flex-col items-center justify-center h-full z-50">
              <motion.a className="text-4xl py-10"
